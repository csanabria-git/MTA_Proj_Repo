...
def ExtractBusTimes (URLDict):
    ...
    for i in URLDict:
        ...
        JSON = response.json()
        for i in range(2):
            #DirectionRef is binary, 0 means Pier6 towards Pier 6 and 1 means Southound towards Bay Ridge
...
@app.route('/')
def my_route():
  return render_template('index.html', NTrains=NTrainDict, DTrains=DTrainDict,RTrains=RTrainDict,B63=BusDict)
